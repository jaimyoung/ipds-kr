...
명사를 사용해서, 이전처럼 코퍼스를 만든다:
```{r}
bill_corpus <- Corpus(VectorSource(nouns))
...
bill_corpus <- tm_map(bill_corpus, removeWords, c(stopwords('english')))
...
```{r}
par(family='Dotum')
wordcloud(bill_corpus, max.words=100, random.order=FALSE,
          colors=brewer.pal(8, "Dark2"))
```
